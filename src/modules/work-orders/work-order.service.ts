...
import { FULFILLMENT_TYPE, WORK_ORDER_STATUS } from "@/shared/constants/work-order-status.js";
...
export class WorkOrderService {
  ...
  async list(
    ...
    const cachedList = await this.cache.get<{
      data: IWorkOrder[];
    ...
    if (options.priority && options.priority !== "all") filter.priority = options.priority;
    if (options.search)
      filter.$or = [{ title: { $regex: options.search, $options: "i" } }, { _id: options.search }];
    ...
    const [data, total] = await Promise.all([
      cursor
        ? this.repository.findCursorPage(
            filter,
  ...
  async listForVendor(actor: Actor, options: { page: number; limit: number }) {
    ...
    const vendorId =
      actor.vendorId ??
      (await User.findById(actor.userId).select("vendorId"))?.vendorId?.toString();
  ...
  async get(id: string, actor: Actor) {
    const isVendor = [ROLES.VENDOR_LEAD, ROLES.VENDOR_MANAGER, ROLES.VENDOR_TECHNICIAN].includes(
      actor.role as
        typeof ROLES.VENDOR_LEAD | typeof ROLES.VENDOR_MANAGER | typeof ROLES.VENDOR_TECHNICIAN,
    );
    if (isVendor) {
      const vendorId =
        actor.vendorId ??
  ...
  async update(id: string, input: UpdateWorkOrderInput, actor: Actor) {
    ...
    if (actor.organizationId)
      await this.cache.delete(cacheKeys.workOrder(actor.organizationId, id));
    if (actor.organizationId)
      await this.cache.deleteByPattern(
        `${cacheKeys.tenantPrefix(actor.organizationId)}work-orders:list:*`,
      );
    return saved;
  }
  ...
  async create(
    ...
    if (!actor.organizationId || actor.organizationId !== data.organizationId) {
      throw new AuthorizationException("Work order organization does not match the current tenant");
    }
    await this.assertHierarchy(data.organizationId, data.facilityId, data.locationId, data.assetId);
  ...
  async createFromServiceRequest(
    ...
    const existing = await this.repository.findByServiceRequestId(data.serviceRequestId);
    if (existing)
      return {
        success: true,
    ...
    try {
      result = await this.create(data, actor, transactionSession);
    } catch (error) {
  ...
  async listOpenForVendor(actor: Actor): Promise<ApplicationResult<IWorkOrder[]>> {
  ...
  async listVendorCandidates(id: string, actor: Actor) {
    ...
    if (
      ...
      throw new BusinessException("Work order is not available for vendor sourcing");
    }
  ...
  async updateProgress(
    ...
    if (!this.isAssignedWorker(workOrder, actor)) {
      throw new AuthorizationException("Only the assigned worker can update progress");
    }
    ...
    if (
      ...
      throw new BusinessException("Work order must be assigned before progress starts");
    }
    ...
    if (
      ...
      throw new BusinessException("Work order must be in progress before completion review");
    }

    workOrder.status = data.status;
    const saved = await this.saveWithOutbox(workOrder, "WorkOrderStatusChanged", actor.userId);
    await this.invalidateDerivedCaches(
      actor.organizationId ?? workOrder.organizationId?.toString(),
    );
  ...
  async transition(
    ...
    const assignedVendor =
      actor.vendorId && isSameObjectId(workOrder.assignedVendorId, actor.vendorId);
    if (
      !this.isAssignedWorker(workOrder, actor) &&
      !(
        [ROLES.VENDOR_LEAD, ROLES.VENDOR_MANAGER].includes(
          actor.role as typeof ROLES.VENDOR_LEAD | typeof ROLES.VENDOR_MANAGER,
        ) && assignedVendor
      )
    )
      throw new AuthorizationException("Only the assigned worker can update progress");
    const allowed: Record<string, string[]> = {
      ...
      in_progress: [WORK_ORDER_STATUS.ON_HOLD, WORK_ORDER_STATUS.PENDING_COMPLETION],
  ...
  async assign(
    ...
    const workOrder = await this.get(id, actor);
    if (
      WORK_ORDER_STATUS.OPEN !== workOrder.status &&
      WORK_ORDER_STATUS.ASSIGNED !== workOrder.status
    ) {
      throw new BusinessException("Work order is not available for internal assignment");
    }
    await this.assertInternalTechnician(data.technicianId, actor.organizationId!);
  ...
  async approveCompletion(
    ...
    if (workOrder.status !== WORK_ORDER_STATUS.PENDING_COMPLETION) {
      throw new BusinessException("Only pending completion work orders can be approved");
    }
  ...
  async rejectCompletion(
    ...
    if (workOrder.status !== WORK_ORDER_STATUS.PENDING_COMPLETION) {
      throw new BusinessException("Only pending completion work orders can be rejected");
    }
  ...
  async requestInformation(
    workOrderId: string,
    data: RequestInformationInput,
    actor: Actor,
  ): Promise<ApplicationResult<IWorkOrder>> {
    this.assertApprovalActor(actor);
    const workOrder = await this.repository.findById(workOrderId);
    if (!workOrder || workOrder.organizationId.toString() !== actor.organizationId)
      throw new NotFoundException("Work order not found");
    if (workOrder.status !== WORK_ORDER_STATUS.PENDING_COMPLETION)
      throw new BusinessException("Only pending completion work orders can request information");
  ...
  private assertManager(actor: Actor) {
    if (!managerRoles.includes(actor.role)) {
      throw new AuthorizationException("Only admin or facility manager can perform this action");
    }
  }
  ...
  private async createWithOutbox(
    ...
    if (transactionSession) {
      const created = await this.repository.create(data, transactionSession);
  ...
  private async assertInternalTechnician(technicianId: string, organizationId: string) {
    const technician = await User.findById(technicianId).select("role organizationId");

    if (!technician || technician.role !== ROLES.TECHNICIAN) {
      throw new ValidationException("Only internal technicians can be assigned");
    }

    if (!isSameObjectId(technician.organizationId, organizationId)) {
      throw new ValidationException("Technician does not belong to this organization");
    }
  ...
  private async assertHierarchy(
    ...
    if (!facility) throw new ValidationException("Facility does not belong to this organization");
    if (locationId) {
      ...
      if (!location) throw new ValidationException("Location does not belong to this facility");
    }
    if (assetId) {
