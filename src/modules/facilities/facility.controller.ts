...
import { createFacilitySchema, listFacilitiesSchema } from "./facility.schema.js";
...
export const listFacilities = requestHandler<AuthRequest>(async (req, res) => {
  ...
  const result = await service.findByOrganizationPaginated(req.user.organizationId ?? "", options);
  if (!result.data) throw new Error("Facility list could not be loaded");
  const page = result.data;

  const facilityIds = page.data.map((facility) => facility.id);
  const facilityObjectIds = facilityIds
  ...
  const [locationCounts, assetCounts, workOrderCounts] = await Promise.all([
    Location.aggregate([
      { $match: { organizationId: organizationObjectId, facilityId: { $in: facilityObjectIds } } },
...
export const getFacilityRelationships = requestHandler<AuthRequest>(async (req, res) => {
  ...
  const [locations, assets, openWorkOrderCount, pmScheduleCount] = await Promise.all([
    Location.find({ facilityId: req.params.facilityId, organizationId: req.user.organizationId })
      .limit(100)
      .lean(),
    Asset.find({ facilityId: req.params.facilityId, organizationId: req.user.organizationId })
      .limit(100)
      .lean(),
  ...
  return res.ok(
    ...
    "Facility relationships retrieved",
  );
});

export const updateFacility = requestHandler<AuthRequest>(async (req, res) => {
  const data = (await import("./facility.schema.js")).updateFacilitySchema.parse(req.body);
  const result = await service.update(req.params.facilityId, data, req.user);
  return res.ok(result.data, result.message);
});

export const deactivateFacility = requestHandler<AuthRequest>(async (req, res) => {
